...
## CLI-runners vs API-runners (важное отличие)

- **CLI** (claude/codex/gemini) — хранят историю сессии у себя, `--resume` восстанавливает контекст после рестарта моста.
- **API** (deepseek) — история живёт в памяти процесса. После рестарта контекст теряется, сессия начинается заново. Нормально для коротких задач; для долгих непрерывных диалогов выбирай CLI-runner.
