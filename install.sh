...
# ───────── Шаг 4.6: анонимные баг-репорты (по умолчанию ВКЛ) ─────────
BUGREPORT="${AIOS_BUGREPORT:-1}"
step_bugreport() {
    b " Шаг 4.6: Анонимные баг-репорты"; hr
    if [[ "$NONINTERACTIVE" == 1 ]]; then
        [[ "$BUGREPORT" == 1 ]] && ok "NI: отправка баг-репортов ВКЛ (по умолчанию)" \
            || ok "NI: отправка выключена (AIOS_BUGREPORT=0) — только локально"
        return 0
    fi
    cat > "$TTY" <<'EOF'
    Анонимные баг-репорты ВКЛючены — это помогает быстрее находить и чинить проблемы.
    Уходит: версия, ОС, тип ошибки, обрезанный стектрейс с заменой путей.
    НЕ уходит: твои файлы, переписка, ключи, личные данные.
    Всё всегда дублируется локально (logs/bugs/). Выключить — сейчас или позже
    (скажи Сисадмину). Подробности — PRIVACY.md.
EOF
    local ans; ask " Оставить отправку включённой? (Y/n): " ans "y"
    case "$ans" in
        [nNнН]|no|No|нет|Нет) BUGREPORT="0"; ok "Выключено — баги только локально" ;;
